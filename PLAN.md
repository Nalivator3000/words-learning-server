...
### 0.0.2 Полная миграция на централизованную систему (IN PROGRESS)
- [ ] **Создать новый translation manager (i18n.js)**
  - [ ] Класс I18nManager с методами t(), setLanguage(), etc.
  - [ ] Загрузка переводов из JSON
  - [ ] Автоматическое обновление DOM элементов
  - [ ] Поддержка интерполяции переменных

- [ ] **Запустить auto-translate для всех 499 текстов**
  - [ ] Запустить: `node scripts/auto-translate.js`
  - [ ] Проверить качество переводов
  - [ ] Вручную исправить критичные неточности
  - [ ] Коммит: translations/source-texts.json (все языки)

- [ ] **Мигрировать HTML файлы**
  - [ ] index.html - добавить data-i18n ко всем элементам
  - [ ] Удалить хардкоженные русские тексты
  - [ ] Протестировать переключение языков

- [ ] **Мигрировать JavaScript файлы**
  - [ ] app.js - заменить строки на i18n.t('key')
  - [ ] gamification.js - заменить строки
  - [ ] analytics.js - заменить строки
  - [ ] quiz.js - заменить строки
  - [ ] survival-mode.js - заменить строки
  - [ ] onboarding.js - заменить строки

- [ ] **Автоматическая проверка переводов**
  - [ ] Скрипт для поиска непереведенных текстов
  - [ ] Coverage report (% переведенных текстов)
  - [ ] CI/CD интеграция (fail если < 95% coverage)
...
- [ ] **Удалить старую систему**
  - [ ] Удалить language-manager.js (после миграции)
  - [ ] Обновить все импорты на i18n.js
