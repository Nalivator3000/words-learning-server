...
async function addWordList() {
    ...
    try {
        ...
        `, [
            ...
            wordListData.from_lang,
            wordListData.to_lang,
            wordListData.category,
            wordListData.difficulty_level,
            wordListData.topic,
            wordListData.words.length,
            true
        ]);
        ...
        // Insert words
        for (let i = 0; i < wordListData.words.length; i++) {
            const word = wordListData.words[i];
            await client.query(`
        ...
        console.log(`✅ Added ${wordListData.words.length} words to collection`);
        ...
        console.error('❌ Error adding word list:', err.message);
        throw err;
    } finally {
        client.release();
        await pool.end();
