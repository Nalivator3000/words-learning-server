...
const KEYWORD_MAPPING = {
  'family': ['父', '母', '親', '子', '息子', '娘', '兄', '弟', '姉', '妹', '祖', '配偶', '夫', '妻', '族', '親戚', '家族'],
  'food': ['ご飯', 'パン', '肉', '魚', '野菜', '果物', '卵', 'チーズ', 'バター', '油', 'スープ', 'サラダ', 'ケーキ', 'デザート', '食', '飲', 'メニュー', 'レストラン', 'カフェ', '料理', 'メシ'],
  'travel': ['駅', '電車', '車', '飛行機', '船', 'バス', '空港', '道路', '地図', '旅', '訪問', '観光', 'パスポート', 'チケット', 'ホテル', '宿泊', '移動', 'ツアー', '道', '街'],
  ...
  'communication': ['話す', '聞く', '言う', 'メッセージ', '手紙', '郵便', 'メール', '質問', '答え', '議論', '意見', '提案', '感謝', '挨拶', 'さようなら', '電話', 'チャット', '会話'],
  'culture': ['芸術', '絵', '彫刻', '音楽', '楽器', 'ピアノ', 'ギター', 'ドラム', '歌う', '歌', '曲', 'ダンス', '演劇', '映画', 'フィルム', '博物館', 'ギャラリー', '歴史', '神社', 'お寺'],
  'emotions': ['happy', '幸せ', '楽しい', '喜び', 'sad', '悲しい', 'angry', '怒り', 'surprised', '驚き', 'scared', '怖い', 'worried', '心配', 'bored', 'つまらない', 'tired', '疲れた', '愛', '感情'],
  'sports': ['サッカー', 'フットボール', 'バスケット', 'バレーボール', '野球', 'テニス', 'ゴルフ', '体操', 'ボクシング', '空手', '柔道', 'スイミング', 'サーフィン', 'スキー', 'スノーボード', 'スケート', 'サイクリング', '自転車', 'ジョギング', '走る'],
  ...
  'shopping': ['店', 'みせ', 'スーパー', 'マーケット', 'デパート', 'ショップ', 'モール', 'レジ', '商品', 'アイテム', '選ぶ', 'バッグ', 'ショッピング', 'カート', '価格', '値段', 'コスト', '割引', 'セール', '販売', 'バーゲン', 'クーポン', 'ギフト', 'レシート', 'チェックアウト', '返品', '支払い']
};
  ...
  return new Promise((resolve, reject) => {
    ...
    rl.on('line', (line) => {
      ...
      if (word.length > 0) {
        ...
        lineCount++;
        if (lineCount % 1000 === 0) {
          console.log(`Processed: ${lineCount}`);
        }
      }
    });
    ...
    rl.on('close', () => {
      ...
      for (const [theme, count] of sorted) {
        const pct = ((count / lineCount) * 100).toFixed(2);
