...
// Import all final expansion files
const a1Final = require('./add-a1-final.js').collections;
const a2Final = require('./add-a2-final.js').collections;
const b1Final = require('./add-b1-final.js').collections;
const b2Final = require('./add-b2-final.js').collections;
const c1Final = require('./add-c1-final.js').collections;
const c2Final = require('./add-c2-final.js').collections;
...
async function importFinalWords() {
  const client = await pool.connect();

  try {
    console.log('🚀 Starting import of final German vocabulary...\n');
    ...
    // Final verification
    console.log('\n' + '='.repeat(60));
    console.log('📊 FINAL VERIFICATION - ru→de COMPLETE');
    console.log('='.repeat(60));
    ...
    if (totalCurrent >= 10800 && allComplete) {
      console.log('\n🎉 SUCCESS! ru→de pair is COMPLETE with 10,800+ words!');
      console.log('📦 Ready to start translations for other language pairs.');
    } else {
      console.log(`\n⏳ Progress: ${totalCurrent}/10,800 words (${Math.round((totalCurrent/10800)*100)}%)`);
      console.log(`   Still needed: ${10800 - totalCurrent} words`);
    }
