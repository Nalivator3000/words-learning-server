...
async function translateWithGoogleAPI(text) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=de&tl=es&dt=t&q=${encodedText}`;
...
async function translateGermanToSpanish(germanWord) {
  // Clean the word
  ...
  // For nouns with articles, try to preserve Spanish article/grammar hints
  // Spanish has articles like German, but we can add gender hints for learners
  if (gender && translation) {
    // Just return the translation without article since Spanish doesn't use articles
...
async function translateAllToSpanish() {
  ...
  try {
    console.log('🇪🇸 Starting German → Spanish translation for ALL words...\n');
    ...
    const existingResult = await client.query(`
      SELECT COUNT(*) as count FROM target_translations_spanish
    ...
    // Fetch all German words that don't have Spanish translations yet
    const words = await client.query(`
      SELECT sw.id, sw.word, sw.level, sw.example_de
      FROM source_words_german sw
      LEFT JOIN target_translations_spanish tp ON tp.source_word_id = sw.id AND tp.source_lang = 'de'
      WHERE tp.id IS NULL
      ORDER BY sw.id
    `);
    ...
    for (let i = 0; i < words.rows.length; i++) {
      ...
      const translation = await translateGermanToSpanish(row.word);
      ...
      if (batchInserts.length >= BATCH_SIZE || i === words.rows.length - 1) {
        if (batchInserts.length > 0) {
          try {
            ...
            await client.query(`
              INSERT INTO target_translations_spanish
    ...
    console.log(`\n✓ Total Spanish translations in database: ${finalCount.toLocaleString()}/${totalWords.toLocaleString()}`);
    ...
    console.log('\n🎉 German → Spanish translation job complete!');
...
// Run the translation
translateAllToSpanish().catch(err => {
