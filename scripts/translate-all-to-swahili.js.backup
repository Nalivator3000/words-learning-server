...
async function translateWithGoogleAPI(text) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=de&tl=pl&dt=t&q=${encodedText}`;
...
async function translateGermanToPolish(germanWord) {
  // Clean the word
  ...
  // For nouns with articles, try to preserve Polish article/grammar hints
  // Polish doesn't have articles like German, but we can add gender hints for learners
  if (gender && translation) {
    // Just return the translation without article since Polish doesn't use articles
...
async function translateAllToPolish() {
  const client = await pool.connect();

  try {
    console.log('🇵🇱 Starting German → Polish translation for ALL words...\n');
    ...
    const existingResult = await client.query(`
      SELECT COUNT(*) as count FROM target_translations_polish
    ...
    // Fetch all German words that don't have Polish translations yet
    const words = await client.query(`
      SELECT sw.id, sw.word, sw.level, sw.example_de
      FROM source_words_german sw
      LEFT JOIN target_translations_polish tp ON tp.source_word_id = sw.id AND tp.source_lang = 'de'
      WHERE tp.id IS NULL
      ORDER BY sw.id
    `);
    ...
    for (let i = 0; i < words.rows.length; i++) {
      ...
      const translation = await translateGermanToPolish(row.word);
      ...
      if (batchInserts.length >= BATCH_SIZE || i === words.rows.length - 1) {
        if (batchInserts.length > 0) {
          try {
            ...
            await client.query(`
              INSERT INTO target_translations_polish
    ...
    console.log(`\n✓ Total Polish translations in database: ${finalCount.toLocaleString()}/${totalWords.toLocaleString()}`);
    ...
    console.log('\n🎉 German → Polish translation job complete!');
...
// Run the translation
translateAllToPolish().catch(err => {
