...
async function translateWithGoogleAPI(text) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=de&tl=en&dt=t&q=${encodedText}`;
...
async function translateGermanToEnglish(germanWord) {
  // Clean the word
  ...
  // For nouns with articles, try to preserve English article/grammar hints
  // English uses articles like German, so we can preserve some context
  if (gender && translation) {
    // Just return the translation without article since we'll handle it in the app
...
async function translateAllToEnglish() {
  ...
  try {
    console.log('🇬🇧 Starting German → English translation for ALL words...\n');
    ...
    const existingResult = await client.query(`
      SELECT COUNT(*) as count FROM target_translations_english
    ...
    // Fetch all German words that don't have English translations yet
    const words = await client.query(`
      SELECT sw.id, sw.word, sw.level, sw.example_de
      FROM source_words_german sw
      LEFT JOIN target_translations_english te ON te.source_word_id = sw.id AND te.source_lang = 'de'
      WHERE te.id IS NULL
      ORDER BY sw.id
    `);
    ...
    for (let i = 0; i < words.rows.length; i++) {
      ...
      const translation = await translateGermanToEnglish(row.word);
    ...
    console.log(`\n✓ Total English translations in database: ${finalCount.toLocaleString()}/${totalWords.toLocaleString()}`);
    ...
    console.log('\n🎉 German → English translation job complete!');
...
// Run the translation
translateAllToEnglish().catch(err => {
