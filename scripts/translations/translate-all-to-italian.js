...
async function translateWithGoogleAPI(text) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=de&tl=it&dt=t&q=${encodedText}`;
...
async function translateGermanToItalian(germanWord) {
  // Clean the word
  ...
  // For nouns with articles, try to preserve Italian article/grammar hints
  // Italian has articles like German, but we can add gender hints for learners
  if (gender && translation) {
    // Just return the translation without article since Italian doesn't use articles
...
async function translateAllToItalian() {
  const client = await pool.connect();

  try {
    console.log('🇮🇹 Starting German → Italian translation for ALL words...\n');
    ...
    const existingResult = await client.query(`
      SELECT COUNT(*) as count FROM target_translations_italian
    ...
    // Fetch all German words that don't have Italian translations yet
    const words = await client.query(`
      SELECT sw.id, sw.word, sw.level, sw.example_de
      FROM source_words_german sw
      LEFT JOIN target_translations_italian tp ON tp.source_word_id = sw.id AND tp.source_lang = 'de'
      WHERE tp.id IS NULL
      ORDER BY sw.id
    `);
    ...
    for (let i = 0; i < words.rows.length; i++) {
      ...
      const translation = await translateGermanToItalian(row.word);
      ...
      if (batchInserts.length >= BATCH_SIZE || i === words.rows.length - 1) {
        if (batchInserts.length > 0) {
          try {
            ...
            await client.query(`
              INSERT INTO target_translations_italian
    ...
    console.log(`\n✓ Total Italian translations in database: ${finalCount.toLocaleString()}/${totalWords.toLocaleString()}`);
    ...
    console.log('\n🎉 German → Italian translation job complete!');
...
// Run the translation
translateAllToItalian().catch(err => {
