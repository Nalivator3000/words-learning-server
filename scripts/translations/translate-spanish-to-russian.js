#!/usr/bin/env node
/**
 * Translate Spanish words to Russian
 * Creates translations for ru→es language pair
 */
...
async function translateSpanishToRussian() {
  const client = await pool.connect();

  try {
    console.log('🇷🇺 Starting Spanish → Russian translation...\n');

    // Get total count of Spanish words
    const countResult = await client.query('SELECT COUNT(*) as total FROM source_words_spanish');
    const totalWords = parseInt(countResult.rows[0].total);
    console.log(`📊 Total Spanish words: ${totalWords.toLocaleString()}\n`);
    ...
    if (existingCount >= totalWords) {
      console.log('✅ All Spanish words already translated to Russian!');
      return;
    }
    ...
    const wordsToTranslate = await client.query(`
      ...
      LEFT JOIN target_translations_russian tt
        ON tt.source_word_id = sw.id AND tt.source_lang = 'es'
      WHERE tt.id IS NULL
      ORDER BY sw.id
    `);
    ...
    for (let i = 0; i < wordsNeedingTranslation.length; i++) {
      ...
      try {
        // Translate
        const translation = await translateWithGoogleAPI(word, 'es', 'ru');
...
translateSpanishToRussian().catch(err => {
